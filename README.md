...
# 2) 翻译（优先复用 work/seed/ 精校词库与 trans_cache.json 缓存，缺的才联网）
cd work && python 02_translate.py   # 默认 Google gtx；额度受限：ZALOPC_ENGINE=mm 或 =bing
...
机翻总有几句不顺眼。改 `work/overrides.json`（按越南语原文 → 中文），
